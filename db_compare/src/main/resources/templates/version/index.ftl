...
<div class="template" id="import_template">
    ...
    <strong>PLSQL导出的CSV格式文件:</strong><br/>
      	Oracle查询语句: SELECT B.TABLE_NAME,B.COLUMN_NAME,B.DATA_TYPE AS COLUMN_TYPE,B.DATA_LENGTH AS COLUMN_SIZE FROM USER_TABLES A RIGHT JOIN USER_TAB_COLUMNS B ON A.TABLE_NAME = B.TABLE_NAME ORDER BY A.TABLE_NAME,B.COLUMN_ID;
 <br />
    </div>
</div>
<script>
