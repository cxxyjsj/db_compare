...
<div class="template" id="import_template">
    ...
    <strong>PLSQL导出的CSV格式文件:</strong><br/>
      	Oracle查询语句: SELECT TABLE_NAME,COLUMN_NAME,DATA_TYPE,DATA_LENGTH FROM USER_TAB_COLUMNS WHERE TABLE_NAME IN(SELECT TABLE_NAME FROM USER_TABLES) ORDER BY TABLE_NAME,COLUMN_ID;
 <br />
    </div>
</div>
<script>
